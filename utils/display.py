...
from util_functions import status2str_dict, get_arr_shape, time_to_str
import cv2
import numpy as np
import time
import traceback
...
display_confidence = hp.display_confidence
def display_fn(shared_dict, display_looptime, detect_model_timestamp, last_out_arr_ctypes, status, detect_best_arr_ctypes, capture_arr2_ctypes, capture_arr3_ctypes, out_tf, start_tf):
    try :
        ...
        while True:
            ...
            if detect_best_arr_copy.sum() > 0:
                predict_best_confidence = detect_best_arr_copy[4]
                ...
                if out_tf.value :
                    ...
                    if display_confidence :
                        text += f"({str(predict_best_confidence.round(2))[:4]})"
                    cv2.putText(capture_arr2_copy, text, (box_x1, box_y1 - out_text_y_margin), cv2.FONT_HERSHEY_SIMPLEX,  out_text_fontsize, (0, 0, 255), text_thickness)
                    ...
                    if display_confidence :
                        text += f"({str(predict_best_confidence.round(2))[:4]})"
                    cv2.putText(capture_arr2_copy, text, (box_x1, box_y1 - start_text_y_margin), cv2.FONT_HERSHEY_SIMPLEX,  start_text_fontsize, (0, 255, 0), text_thickness)
                    if display_wheel_point :
                    ...
                    if display_confidence :
                        text += f"({str(predict_best_confidence.round(2))[:4]})"
                    cv2.putText(capture_arr2_copy, text, (box_x1, box_y1 - inside_text_y_margin), cv2.FONT_HERSHEY_SIMPLEX,  inside_text_fontsize, (255, 0, 0), text_thickness)
                    if display_wheel_point :
